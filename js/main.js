...
async function getWeather(city) {
  try {
    const response = await fetch(
      `https://api.weatherapi.com/v1/forecast.json?key=5f39efba01934aeea2703503241212&q=${city}&days=3`
    );
...
function displayWeather(data) {
  function getDateName(dateNum) {
    ...
    const dateText = date.getDay();
    return days[dateText];
  }

  function getDateAndMonth(dateNum) {
    const date = new Date(dateNum);
    const dayNum = date.getDate();
    const monthText = date.getMonth();
    return `${dayNum} ${months[monthText]}`;
  }
